...
PROJECT="$BUILDDIR/../../mcmbuilds/project-$1"
...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../mcmbuilds/project-$1 org.byteball.wallet MCM
	checkOK
...
if $DBGJS
then
	echo "${OpenColor}${Green}* Generating mcm bundle (debug js)...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova
	checkOK
else
	echo "${OpenColor}${Green}* Generating mcm bundle...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova-prod
	checkOK
fi
...
if [ $CURRENT_OS == "ANDROID" ]; then
	...
	cat $BUILDDIR/android/android.css >> $PROJECT/www/css/mcm.css
