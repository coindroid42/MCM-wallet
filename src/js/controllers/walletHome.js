...
angular.module('copayApp.controllers')
	.controller('walletHomeController', function($scope, $rootScope, $timeout, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, correspondentListService, newVersion, autoUpdatingWitnessesList, go, aliasValidationService) {
		...
		this.protocol = conf.program.replace(/byteball/i, 'mcm');
		...
		var disablePaymentRequestListener = $rootScope.$on('paymentRequest', function(event, address, amount, asset, recipient_device_address) {
			...
			if (form.address && form.address.$invalid && !self.blockUx) {
				...
				self.resetForm();
				self.error = gettext('Could not recognize a valid MCM QR Code');
			}*/
		});
		...
		this.shareAddress = function(addr) {
			if (isCordova) {
				...
				window.plugins.socialsharing.shareWithOptions({message: "My MCM address " + self.protocol +  ':' + addr, subject: "My MCM address"/*, url: self.protocol +  ':' + addr*/}, function(){}, function(){});
			}
		};
		...
		this.openCustomizedAmountModal = function(addr) {
			...
			var ModalInstanceCtrl = function($scope, $modalInstance) {
				...
				$scope.protocol = conf.program.replace(/byteball/i, 'mcm');
		...
		function getShareMessage(amount, mnemonic, asset) {
			...
			return {
				message: "Here is your " + (is_private ? "file" : "link") + " to receive " + amount + " " + asset + usd_amount_str + (is_private ? ".  If you don't have a MCM wallet yet, install it from https://mcm.org." : (": https://mcm.org/#textcoin?" + mnemonic)),
				subject: "MCM user beamed you money"
			}
		}
				...
				profileService.requestTouchid(function(err) {
					...
					if (self.binding) {
						if (isTextcoin) {
							delete self.current_payment_key;
							indexScope.setOngoingProcess(gettext('sending'), false);
							return self.setSendError("you can send bound payments to MCM adresses only");
						}
		...
		this.getPrivatePayloadSavePath = function(cb) {
			var fileName = 'MCMPayment-' + $filter('date')(Date.now(), 'yyyy-MM-dd-HH-mm-ss') + '.' + configService.privateTextcoinExt;
			if (!isCordova) {
		...
		this.openInExplorer = function(unit) {
			...
			var url = 'https://' + testnet + 'explorer.mcm.org/#' + unit;
