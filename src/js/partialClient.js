...
function initWallet() {
	...
	function setWalletNameAndColor(walletName) {
		...
		var color = root.config.colorFor ? root.config.colorFor[root.focusedClient.credentials.walletId] : '#134605';
		if(!color) color = '#134605';
	...
	function setWalletsInMenu() {
		...
		for (var key in root.walletClients) {
			...
			html += '<li onclick="wallet.selectWallet(\'' + walletId + '\')" id="w' + walletId + '" class="nav-item ' + (walletId === selectedWalletId ? 'selected' : '') + '">' +
				'<a class="oh"><div class="avatar-wallet " style="background-color: ' + (colors && colors[walletId] ? colors[walletId] : '#134605') + '">' + credentials.walletName.substr(0, 1) + ' </div>' +
	...
	function loadCompleteClient(showClient) {
		...
		angularJs.onload = function() {
			var mcmJS = document.createElement('script');
			mcmJS.src = 'mcm.js';
			body.appendChild(mcmJS);
			mcmJS.onload = function() {
	...
	function formatAmount(mcm, asset) {
		...
		return utils.formatAmount(mcm, unitCode) + ' ' + name;
	}
